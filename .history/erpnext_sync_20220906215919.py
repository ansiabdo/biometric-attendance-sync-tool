...
allowlisted_errors = [EMPLOYEE_NOT_FOUND_ERROR_MESSAGE,
                      EMPLOYEE_INACTIVE_ERROR_MESSAGE, DUPLICATE_EMPLOYEE_CHECKIN_ERROR_MESSAGE]
...
def main():
    ...
    try:
        ...
        if last_lift_off_timestamp:
            print("last_lift_off_timestamp = %s" % last_lift_off_timestamp)
            ...
            for device in config.devices:
                ...
                if os.path.exists(dump_file):
                    ...
                    with open(dump_file, 'r') as f:
                        ...
                        if file_contents:
                            device_attendance_logs = list(map(lambda x: _apply_function_to_key(
                                x, 'timestamp', datetime.datetime.fromtimestamp), json.loads(file_contents)))
                try:
                    print("Try :" + device['device_id'])
                    pull_process_and_push_data(device, device_attendance_logs)
                    status.set(f'{device["device_id"]}_push_timestamp', str(
                        datetime.datetime.now()))
                    if os.path.exists(dump_file):
                        os.remove(dump_file)
                    info_logger.info(
                        "Successfully processed Device: " + device['device_id'])
                    print("Successfully processed Device: " +
                          device['device_id'])
                except:
                    error_logger.exception(
                        'exception when calling pull_process_and_push_data function for device'+json.dumps(device, default=str))
            if hasattr(config, 'shift_type_device_mapping'):
                update_shift_last_sync_timestamp(
                    config.shift_type_device_mapping)
            status.set('mission_accomplished_timestamp',
                       str(datetime.datetime.now()))
            info_logger.info("Mission Accomplished!")
            print("Mission Accomplished!")
    except BaseException as e:
        error_logger.exception(
            'exception has occurred in the main function...')
        print(e)
...
def pull_process_and_push_data(device, device_attendance_logs=None):
    ...
    """
    print("Started pull_process_and_push_data   " + device['device_id'])
    ...
    attendance_success_logger = setup_logger(attendance_success_log_file, '/'.join(
        [config.LOGS_DIRECTORY, attendance_success_log_file])+'.log')
    attendance_failed_logger = setup_logger(
        attendance_failed_log_file, '/'.join([config.LOGS_DIRECTORY, attendance_failed_log_file])+'.log')
    print("device_attendance_logs")
    if not device_attendance_logs:
        print("11 if not device_attendance_logs")
        device_attendance_logs = get_all_attendance_from_device(
            device['ip'], device_id=device['device_id'], clear_from_device_on_fetch=device['clear_from_device_on_fetch'])
        if not device_attendance_logs:
            print("22 if not device_attendance_logs")
            return
    ...
    print("import_start_date = " + str(import_start_date))

    if last_line or import_start_date:
        print("if import_start_date=")
        last_user_id = None
        last_timestamp = None
        if last_line:
            print("if last_line=")
        ...
        for i, x in enumerate(device_attendance_logs):
            if last_user_id and last_timestamp:
                ...
                # print('x[timestamp] : ' + str(x['timestamp']))
                if x['timestamp'] >= last_timestamp:
                    index_of_last = i
                    print("for i,x = " + str(i) + x['user_id'])
                    print('x[timestamp] : ' + str(x['timestamp']))
                    break

    print("index_of_last : " + str(index_of_last))
    # Setup the shift time here
    for device_attendance_log in device_attendance_logs[index_of_last+1:]:
        # print("for device_attendance_log in ")

        #   device_attendance_log['user_id'] + "  " + str(device_attendance_log['timestamp']) + " " + device_attendance_log['punch'])

        punch_direction = device['punch_direction']
        punch_hour = device_attendance_log['timestamp']
        print('ID + punch_hour is :' +
              device_attendance_log['user_id']+'   -   '+punch_hour.__str__())
        ...
        try:
            ...
            print("erpnext_status_code = " + str(erpnext_status_code))
            if erpnext_status_code == 200:
                attendance_success_logger.info("\t".join([erpnext_message, str(device_attendance_log['uid']),
                                                          str(device_attendance_log['user_id']), str(
                    device_attendance_log['timestamp'].timestamp()),
                    str(device_attendance_log['punch'] or "--"), str(
                    device_attendance_log['status']),
...
def get_all_attendance_from_device(ip, port=4370, timeout=30, device_id=None, clear_from_device_on_fetch=False):
    ...
    try:
        ...
        info_logger.info(
            "\t".join((ip, "Device Disable Attempted. Result:", str(x))))
        print("\t".join((ip, "Device Disable Attempted. Result:", str(x))))
        attendances = conn.get_attendance()
        info_logger.info(
            "\t".join((ip, "Attendances Fetched:", str(len(attendances)))))
        ...
        if len(attendances):
            ...
            with open(dump_file_name, 'w+') as f:
                f.write(json.dumps(list(map(lambda x: x.__dict__,
                        attendances)), default=datetime.datetime.timestamp))
            if clear_from_device_on_fetch:
                x = conn.clear_attendance()
                info_logger.info(
                    "\t".join((ip, "Attendance Clear Attempted. Result:", str(x))))
                print("\t".join((ip, "Attendance Clear Attempted. Result:", str(x))))
        x = conn.enable_device()
        info_logger.info(
            "\t".join((ip, "Device Enable Attempted. Result:", str(x))))
        print("\t".join((ip, "Device Enable Attempted. Result:", str(x))))
    except:
        error_logger.exception(
...
def send_to_erpnext(employee_field_value, timestamp, device_id=None, log_type=""):
    """
    Example: send_to_erpnext('12349',datetime.datetime.now(),'HO1','IN')
    """
    print("send_to_erpnext : " + employee_field_value + " " +
          str(timestamp) + " - " + device_id + " - " + log_type)
    ...
    if response.status_code == 200:
        ...
        if EMPLOYEE_NOT_FOUND_ERROR_MESSAGE in error_str:
            error_logger.error('\t'.join(['Error during ERPNext API Call.', str(employee_field_value), str(
                timestamp.timestamp()), str(device_id), str(log_type), error_str]))
            # TODO: send email?
        else:
            error_logger.error('\t'.join(['Error during ERPNext API Call.', str(employee_field_value), str(
                timestamp.timestamp()), str(device_id), str(log_type), error_str]))
    ...
    for shift_type_device_map in shift_type_device_mapping:
        ...
        for device_id in shift_type_device_map['related_device_id']:
            if not status.get(f'{device_id}_push_timestamp'):
                ...
                break
            pull_timestamp_array.append(_safe_convert_date(status.get(
                f'{device_id}_pull_timestamp'), "%Y-%m-%d %H:%M:%S.%f"))
        if all_devices_pushed:
            ...
            for shift in shift_type_device_map['shift_type_name']:
                try:
                    sync_current_timestamp = _safe_convert_date(status.get(
                        f'{shift}_sync_timestamp'), "%Y-%m-%d %H:%M:%S.%f")
                    if (sync_current_timestamp and min_pull_timestamp > sync_current_timestamp) or (min_pull_timestamp and not sync_current_timestamp):
                        ...
                        if response_code == 200:
                            status.set(f'{shift}_sync_timestamp',
                                       str(min_pull_timestamp))
                except:
                    error_logger.exception(
                        'Exception in update_shift_last_sync_timestamp, for shift:'+shift)
...
def send_shift_sync_to_erpnext(shift_type_name, sync_timestamp):
    ...
    try:
        ...
        if response.status_code == 200:
            info_logger.info("\t".join(['Shift Type last_sync_of_checkin Updated', str(
                shift_type_name), str(sync_timestamp.timestamp())]))
            print("\t".join(['Shift Type last_sync_of_checkin Updated', str(
                shift_type_name), str(sync_timestamp.timestamp())]))
        else:
            error_str = _safe_get_error_str(response)
            error_logger.error('\t'.join(['Error during ERPNext Shift Type API Call.', str(
                shift_type_name), str(sync_timestamp.timestamp()), error_str]))
        return response.status_code
    except:
        error_logger.exception("\t".join(['exception when updating last_sync_of_checkin in Shift Type', str(
            shift_type_name), str(sync_timestamp.timestamp())]))
...
info_logger = setup_logger(
    'info_logger', '/'.join([config.LOGS_DIRECTORY, 'logs.log']))
